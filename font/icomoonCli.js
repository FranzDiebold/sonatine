// This IcoMoon.io CLI script is a modification of https://github.com/BernhardBehrendt/icomoon-cli
...
async function pipeline(options = {}) {
  try {
    ...
    const browser = await puppeteer.launch({headless: !visible, args: ['--no-sandbox', '--disable-setuid-sandbox']});
    ...
    await sleep(20000);
    await page.waitForSelector(PAGE.FIRST_ICON_BOX, {timeout: 0});
    await sleep(10000);
        ...
        request.onsuccess = function () {
          ...
          keys.onsuccess = function () {
            ...
            main.onsuccess = function () {
              ...
              names.forEach((glyphNames, idx) => {
                data.obj.iconSets[0].selection[idx].name = 'glyph-' + idx;

                const characters = glyphNames.filter(n => n.length === 1);
                if (characters.length > 0) {
                  data.obj.iconSets[0].selection[idx].code = characters[0].charCodeAt(0);
                } else {
                  data.obj.iconSets[0].selection[idx].code = 2**16 - 2 - idx;
                }

                const ligatures = glyphNames.filter(n => n.length > 1).join(', ');
                if (ligatures) {
                  data.obj.iconSets[0].selection[idx].ligatures = ligatures;
                }
              });
              store.put(data);
            };
    ...
    await page.waitForSelector(PAGE.DOWNLOAD_BUTTON, {timeout: 0});
