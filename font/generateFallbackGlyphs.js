...
const digits = _.range(10).map((digit) => digit.toString());
const alphabet = _.range(65, 91)
...
const allCharsInLigatures = new Set(
  getAllGlyphNames()
    .flatMap((ligatureEquivs) =>
      ligatureEquivs.flatMap((ligature) => [...ligature])
    )
    .concat(classicFallbackChars)
);
const characters = new Set(
  getAllGlyphNames().flatMap((ligatureEquivs) =>
    ligatureEquivs.filter((ligature) => ligature.length === 1)
  )
);
...
const generateFallbackGlyphs = () => {
  ...
  fs.readFile("res/open-sans-v18-latin-700.svg", function (err, data) {
    parser.parseString(data, function (err, result) {
      ...
      const glyphNames = characterDifference.map((char, idx) => {
      ...
      fs.writeFile(
        `${basePath}/fallbackGlyphs.json`,
        JSON.stringify({
          icons: glyphsAndNamesList[0],
          names: glyphsAndNamesList[1],
        }),
        (err) => {
          if (err) {
            console.error(err);
