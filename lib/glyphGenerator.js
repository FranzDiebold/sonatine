...
const drawAccidental = (glyph, noteLine, accidental) => {
  ...
  return drawObjectOnLine(
    glyph,
    objectName,
    0,
    noteLine,
    false,
    objectName === "flat" ? -16 : 0
  ).right;
};
...
const drawStemAndFlags = (glyph, noteDimensions, durationName, rotate) => {
  ...
  const stemPath = glyph
    .rect(stemWidth, stemLength)
    .fill(color)
    .transform({
      translate: {
  ...
  if (numFlags > 0) {
    ...
    const flagX =
      noteDimensions.left + (rotate ? stemWidth : noteDimensions.width);
    ...
    mainFlagPath.transform({
      translate: {
        ...
        y:
          flagBaseY -
          (rotate ? mainFlagPath.height() : 0) +
          (rotate ? -1 : 1) * numSubFlags * subFlagDY,
      },
      flip: flipFlag,
    });
  }
...
const drawDot = (glyph, noteLine, noteDimensions, rotate) => {
  ...
  glyph
    .circle(dotDiameter)
    .fill(color)
    .move(dotX, noteDimensions.yCenter - dotDiameter / 2 + dotDY);
};

const drawArticulation = (
  glyph,
  articulation,
  noteLine,
  noteDimensions,
  ...
  if (articulation === "!" || articulation === "_") {
    const dLineY =
      (rotate ? 1 : -1) *
      (noteLine % 2 === 0 && noteLine > 0 && noteLine < 8 ? 3 : 2);
    const y = baseY - halfLineHeight * (noteLine + dLineY);
    if (articulation === "!") {
      glyph
        .circle(dotDiameter)
        .fill(color)
        .move(x - dotDiameter / 2, y - dotDiameter / 2);
    } else {
      const width = 25;
      const height = 2;
      glyph
    ...
    const objectPath = glyph
      .path(paths[objectNameMap[articulation]])
      .stroke({ color: color, width: 0 });
...
const drawNote = (glyph, noteAttributes, margin = defaultMargin) => {
  const noteNameToNumber = [..."cdefgah"].reduce(
    (nameToNumberMap, note, idx) => {
      return {
        [note]: idx,
        ...nameToNumberMap,
      };
  ...
  const noteLine =
    noteValue + Object.keys(noteNameToNumber).length * (octave - 1) - 2;
  ...
  if (noteAttributes.accidental) {
    ...
    accidentalRight =
      drawAccidental(group, noteLine, noteAttributes.accidental) +
      accidentalDistance;
  }
  const noteObjectName = paths.hasOwnProperty(`${duration}Note`)
    ? `${duration}Note`
    : "vNote";
  const noteDimensions = drawObjectOnLine(
    group,
    noteObjectName,
    accidentalRight,
    noteLine,
...
const drawRest = (glyph, durationName, dotted, margin = defaultMargin) => {
  ...
  if (durationName === "g" || durationName === "h") {
    ...
    right = drawCenteredObject(glyph, "quarterRest", -2, false).right;
  } else {
    const duration = durationNameToDuration[durationName];
    const numQuaverRests = Math.max(Math.log2(duration) - 2, 0);
    const startLine =
      numQuaverRests !== 2 ? 2 * (3 - Math.ceil(numQuaverRests / 2)) : 2;
    right = _.range(numQuaverRests).reduce((_, idx) => {
      const currentLine = startLine + 2 * idx;
      const currentRight = drawObjectOnLine(
        glyph,
        "quaverRest",
        margin + idx * 7.05,
        currentLine,
  ...
  if (dotted) {
    ...
    glyph
      .circle(dotDiameter)
      .fill(color)
      .move(dotX, baseY - 5 * halfLineHeight - dotDiameter / 2);
...
const drawBarline = (glyph, numBarlines) => {
  var width;
  glyph
    .rect(2, staffHeight + 2)
    .fill(color)
    .move(0, baseY - staffHeight - 1);
  if (numBarlines === 2) {
    width = 20;
    glyph
      .rect(2, staffHeight)
      .fill(color)
      .move(width - 2, baseY - staffHeight);
  } else if (numBarlines === 3) {
    width = 19;
    const thickLineWidth = 10;
    const thinLineWidth = 2;
    glyph
      .rect(thickLineWidth, staffHeight)
      .fill(color)
      .move(width - thickLineWidth, baseY - staffHeight);
    glyph
      .rect(thinLineWidth, staffHeight)
      .fill(color)
      .move(0, baseY - staffHeight);
  }
  if (width) {
...
const drawKeySignature = (glyph, keySignature, margin = defaultMargin) => {
  ...
  _.range(1, 8).forEach((i) => {
    keySignatureToSymbolsMap[`${i}#`] = i;
    keySignatureToSymbolsMap[`${i}b`] = -i;
    keySignatureToSymbolsMap["#".repeat(i)] = i;
    if (i > 1) {
      keySignatureToSymbolsMap["b".repeat(i)] = -i;
    }
  });
...
const drawRepeatSign = (glyph, repeatSign, margin = defaultMargin) => {
  ...
  glyph
    .rect(thickLineWidth, staffHeight)
    .fill(color)
    .move(left ? 0 : width - thickLineWidth, baseY - staffHeight);
  glyph
    .rect(thinLineWidth, staffHeight)
    .fill(color)
    .move(
      left ? thinLineDx : width - thinLineDx - thinLineWidth,
      baseY - staffHeight
    );
  const dotX = left
    ? thinLineDx + dotDiameter
    : width - thinLineDx - 2 * dotDiameter;
  glyph
    .circle(dotDiameter)
    .fill(color)
    .move(dotX, baseY - 3 * halfLineHeight - dotDiameter / 2);
  glyph
    .circle(dotDiameter)
    .fill(color)
    .move(dotX, baseY - 5 * halfLineHeight - dotDiameter / 2);
...
const generateGlyph = (canvas, glyphStr, margin = defaultMargin) => {
  ...
  const smallNoteNameAccidentalRegexes = [
    "(?<name>[cdefgah])(?<accidental>(es)|(is))?",
    "(?<name>[ea])(?<accidental>s)?",
    "(?<name>[b])",
    "(?<accidental>[#b§]?)(?<name>[cdefgah])",
  ];
  const smallNoteOctaveRegex = "(?<octave>[0-3])?";
  const smallNoteNameAccidentalOctaveRegexes =
    smallNoteNameAccidentalRegexes.map(
      (smallNoteNameRegex) => smallNoteNameRegex + smallNoteOctaveRegex
    );
  const largeNoteNameAccidentalRegexes = [
    "(?<name>[CDEFGAH])(?<accidental>(es)|(is))?",
    "(?<name>[EA])(?<accidental>s)?",
    "(?<name>[B])",
    "(?<accidental>[#b§]?)(?<name>[CDEFGAH])",
  ];
  const largeNoteOctaveRegex = "(?<octave>[0-1])?";
  const largeNoteNameAccidentalOctaveRegexes =
    largeNoteNameAccidentalRegexes.map(
      (largeNoteNameRegex) => largeNoteNameRegex + largeNoteOctaveRegex
    );

  const noteDurationRegex = "(?<duration>[ghva8]|16|32|64|128|256)?";
  const noteArticulationRegex = "(?<articulation>[!_?>])?";
  const noteRegexes = smallNoteNameAccidentalOctaveRegexes
    .concat(largeNoteNameAccidentalOctaveRegexes)
    .map(
      (noteNameOctaveRegex) =>
        new RegExp(
          "^" +
            ...
            noteDurationRegex +
            dottedRegex +
            noteArticulationRegex +
            "$"
        )
    );
  const noteMatch = noteRegexes
    .map((noteRegex) => glyphStr.match(noteRegex))
    .reduce((allMatch, currentMatch) => allMatch || currentMatch, undefined);

  const restRegex = new RegExp(
    "^p(?<duration>[g1h2v4a8]|16|32|64|128|256)?" + dottedRegex + "$",
    "i"
  );
  const restMatch = glyphStr.match(restRegex);

  const barlineRegex = /^[/]{1,3}|[|]{1,3}$/;
  const barlineMatch = glyphStr.match(barlineRegex);

  const clefRegex = /^(?<type>[vgfbcn])s$/i;
  const clefMatch = glyphStr.match(clefRegex);

  const timeSignatureRegex =
    /^(?<times>31|32|22|24|34|44|54|64|74|84|28|38|48|58|68|78|88|98)(t|takt)?$/i;
  const timeSignatureMatch = glyphStr.match(timeSignatureRegex);

  const keySignatureRegex =
    /^([defgahb]|cis|fis|ces|des|es|ges|as)dur|([cdefghb]|cis|dis|fis|gis|ais|es|as)moll|[1-7]#|[#]{1,7}|[1-7]b|[b]{2,7}$/i;
  ...
  if (glyphStr === "-") {
    ...
    drawRest(
      glyph,
      restMatch.groups.duration || defaultRestDuration,
      restMatch.groups.dotted,
      margin
    );
    ...
    glyphStr.toLowerCase() === "ab" ||
    glyphStr.toLowerCase() === "allabreve"
  ) {
