...
const drawRest = (glyph, durationName, dotted, margin = defaultMargin) => {
  ...
  if (durationName === 'g' || durationName === 'h') {
    ...
    const startLine = numQuaverRests !== 2 ? 2 * (3 - Math.ceil(numQuaverRests / 2)) : 2;
...
const drawBarline = (glyph, numBarlines) => {
  var width;
  glyph.rect(2, staffHeight + 2).fill(color).move(0, baseY - staffHeight - 1);
  if (numBarlines === 2) {
    width = 20;
...
const generateGlyph = (canvas, glyphStr, margin = defaultMargin) => {
  ...
  const smallNoteNameAccidentalOctaveRegexes = smallNoteNameAccidentalRegexes
    .map(smallNoteNameRegex => smallNoteNameRegex + smallNoteOctaveRegex);
  ...
  const largeNoteNameAccidentalOctaveRegexes = largeNoteNameAccidentalRegexes
    .map(largeNoteNameRegex => largeNoteNameRegex + largeNoteOctaveRegex);

  const noteDurationRegex = '(?<duration>[ghva8]|16|32|64|128|256)?';
  const noteArticulationRegex = '(?<articulation>[!_?>])?';
  const noteRegexes = smallNoteNameAccidentalOctaveRegexes.concat(largeNoteNameAccidentalOctaveRegexes)
    .map(noteNameOctaveRegex => new RegExp('^' + noteNameOctaveRegex + noteDurationRegex + dottedRegex + noteArticulationRegex + '$'));
  ...
  const keySignatureRegex = /^([defgahb]|cis|fis|ces|des|es|ges|as)dur|([cdefghb]|cis|dis|fis|gis|ais|es|as)moll|[1-7]#|[#]{1,7}|[1-7]b|[b]{2,7}$/i;
